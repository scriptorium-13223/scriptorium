...
|---|---|---|
| Frontend | Vanilla HTML/CSS/JS (no build step) | GitHub Pages (free) |
| Backend | Node.js + Express | Render (free tier) |
| Database | Supabase Postgres (Free Plan) | Supabase |
| File Storage | Supabase Storage (Free Plan) | Supabase |
| Page Counting | pdf-lib / pdf-parse + Tesseract.js OCR | Runs on backend |
| Order Management | Telegram Bot API | Telegram |
| Payment | Cash on Delivery (no gateway) | N/A |
...
2. Once created, go to **SQL Editor** → paste the entire contents of `database/schema.sql` → **Run**.
3. Go to **Storage** → **New Bucket** → name it exactly `assignment-uploads` → set **Private** (not public).
...
2. Follow the prompts to name your bot. BotFather gives you a **bot token** → this is `TELEGRAM_BOT_TOKEN`.
...
1. Push the `backend/` folder to a GitHub repo (or the whole `scriptorium/` repo — Render lets you set a root directory).
...
3. Settings:
   ...
   - **Instance Type:** Free
4. Add all variables from `backend/.env.example` under **Environment**.
5. Deploy. Copy your live URL (e.g. `https://scriptorium-backend.onrender.com`).
...
1. In `frontend/assets/config.js`, set:
   ```js
   const API_BASE_URL = "https://scriptorium-backend.onrender.com/api";
   ```
2. Push the `frontend/` folder contents to a GitHub repo (e.g. as the root, or `/docs` folder).
3. Repo → **Settings → Pages** → Source: deploy from branch → select `main` and `/ (root)` (or `/docs`).
4. Your site will be live at `https://yourusername.github.io/your-repo/`.
5. Go back to Render → update `ALLOWED_ORIGINS` env var to this exact URL → redeploy backend.
...
| `SUPABASE_STORAGE_BUCKET` | Defaults to `assignment-uploads` |
| `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` | From BotFather setup above |
| `ADMIN_SECRET` | Password for the admin panel (choose something strong) |
| `MAX_UPLOAD_SIZE_MB` | Defaults to 20 |
...
|---|---|---|
| POST | `/api/upload/preview-count` | Upload files → get auto-detected page count (no storage) |
| POST | `/api/coupons/validate` | Validate a coupon code, get discount preview |
| POST | `/api/orders` | Create order (files + all fields) → returns order confirmation + invoice |
| POST | `/api/admin/login` | Exchange admin secret for a session token |
| GET | `/api/admin/orders?status=` | List orders (optional status filter) |
| GET | `/api/admin/orders/:id/file-url` | Signed temporary URL to the uploaded file |
| PATCH | `/api/admin/orders/:id/status` | Set status to `pending`/`processing` |
| DELETE | `/api/admin/orders/:id` | Mark delivered → purge order + file permanently |
...
| Issue | Fix |
|---|---|
| CORS error in browser console | Check `ALLOWED_ORIGINS` on Render exactly matches your GitHub Pages URL (no trailing slash) |
| Telegram message not received | Re-check `TELEGRAM_CHAT_ID` — must message the bot first before calling `getUpdates` |
| "File too large" on small files | Check `MAX_UPLOAD_SIZE_MB` env var is set correctly on Render |
| Page count seems wrong on scanned PDF | Encrypted/corrupted PDFs fall back to `pdf-parse`; if both fail, re-export the PDF |
| Backend takes ~40s to respond first time | Normal Render free-tier cold start — consider a free uptime-ping service if this matters to you |
| Admin login fails | Double check `ADMIN_SECRET` matches exactly (case-sensitive) between Render env and what you type |
| Order created but Supabase row missing | Check Render logs — likely an RLS/service-role key issue; confirm you used the `service_role` key, not `anon` |
